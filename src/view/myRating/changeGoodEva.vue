	...
	<div class="rate">
		<div class='rateCon'>
			<div class='rateGoodLeft'>
				<img :src="changeData.thumb" alt="">
			</div>
			<div  class='rateGoodRight'>
				<p>{{changeData.title}}</p>
				<p class='price'>￥{{changeData.price}}</p>
			</div>
		</div>
		...
		<p @click='release' class='release'>发布</p>
	</div>
</template>
<script>
import {commentChange} from '../../http/api.js'
...
export default {
    ...
    methods: {
    	...
    	release(){
    		let info={
    			comment_id:this.changeData.comment_id,
    			content:this.textAreaData,
    			images:this.imageArr.join(','),
    		}
    		commentChange(info).then((response)=>{
...
/*    	upload (e) {  
    ...
    created(){
    	this.changeData=this.$route.query.items
    }
}	
</script>
<style lang='less'>
